    ...
    try {
      ...
      if (recording.thumbnail_url) {
        ...
        if (pathMatch) {
          ...
          if (deleteError) {
            logger.warn(
              'Failed to delete thumbnail from storage (continuing anyway)',
              {
                context: { requestId, recordingId },
      ...
      return successResponse(
        { success: true, message: 'Thumbnail removed' },
        requestId,
      );
    } catch (error: any) {
...
export const GET = apiHandler(async () => {
  return errors.badRequest(
    'Method not allowed. Use PUT to update or DELETE to remove thumbnail.',
  );
});
