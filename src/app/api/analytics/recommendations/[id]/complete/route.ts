...
export const POST = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    ...
    const [{ id: recommendationId }, body] = await Promise.all([
      context.params,
      parseBody(request, recommendationCompleteSchema),
    ]);
    ...
    if (fetchError) {
      console.error(
        '[Complete Recommendation] Error fetching current data:',
        fetchError,
      );
    ...
    if (!recommendation) {
      throw new Error(
        'Recommendation could not be completed. It may not be in progress.',
      );
    }
