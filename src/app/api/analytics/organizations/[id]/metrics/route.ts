...
export const GET = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    const { orgId, org, orgError } = await Promise.all([
      requireSystemAdmin(),
      context.params,
    ]).then(([, { id: orgId }]) =>
      supabaseAdmin
        ...
        .eq('id', orgId)
        .single()
        .then(({ data: org, error: orgError }) => ({ orgId, org, orgError })),
    );
    ...
    const { data: recordings } = await supabaseAdmin
      .from('content')
      .select(
        'file_size, storage_tier, storage_provider, compression_stats, mime_type',
      )
      .eq('org_id', orgId)
      .is('deleted_at', null);
    ...
    // Calculate totals
    const totalStorage =
      recordings?.reduce((sum, r) => sum + (r.file_size || 0), 0) || 0;
    ...
    const providerDistribution = {
      supabase:
        recordings?.filter((r) => r.storage_provider === 'supabase').length ||
        0,
      cloudflare_r2:
        recordings?.filter((r) => r.storage_provider === 'r2').length || 0,
    };

    // Dominant tier (most recordings)
    const dominantTier = (Object.entries(tierDistribution).reduce<
      [string, number] | null
    >(
      (current, entry) => (!current || entry[1] > current[1] ? entry : current),
      null,
    )?.[0] || 'hot') as 'hot' | 'warm' | 'cold' | 'glacier';
    ...
    const compressedFiles =
      recordings?.filter((r) => r.compression_stats != null) || [];
    ...
    // Average file size
    const avgFileSize =
      totalRecordings > 0 ? totalStorage / totalRecordings : 0;
