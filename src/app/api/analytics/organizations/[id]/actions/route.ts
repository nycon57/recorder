...
export const POST = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    ...
    const [adminUser, { id: orgId }, body] = await Promise.all([
      requireSystemAdmin(),
      context.params,
      parseBody(request, organizationActionSchema),
    ]);
    ...
    switch (action) {
      case 'compress': {
        ...
        if (insertError) {
          console.error(
            '[Organization Actions] Error queuing compression jobs:',
            insertError,
          );
      ...
      case 'migrate': {
        ...
        if (insertError) {
          console.error(
            '[Organization Actions] Error queuing migration jobs:',
            insertError,
          );
      ...
      case 'cleanup': {
        ...
        if (updateError) {
          console.error(
            '[Organization Actions] Error deleting files:',
            updateError,
          );
      ...
      case 'report': {
        ...
        const { data: recordings } = await supabaseAdmin
          .from('content')
          .select(
            'file_size, storage_tier, compression_stats, created_at, mime_type',
          )
          .eq('org_id', orgId)
          .is('deleted_at', null);
        ...
        const tierBreakdown = {
          ...
          warm:
            recordings?.filter((r) => r.storage_tier === 'warm').length || 0,
          cold:
            recordings?.filter((r) => r.storage_tier === 'cold').length || 0,
        };
        ...
        // Add recommendations
        const uncompressed =
          recordings?.filter((r) => !r.compression_stats) || [];
