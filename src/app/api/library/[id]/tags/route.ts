...
interface ContentTagWithTagRow {
  created_at: string;
  tags: {
    id: string;
    name: string;
...
export const GET = apiHandler(
  async (request: NextRequest, { params }: RouteParams) => {
    ...
    const formattedTags = (
      (tags as unknown as ContentTagWithTagRow[] | null) ?? []
    ).flatMap((t) =>
      t.tags ? [{ ...t.tags, assigned_at: t.created_at }] : [],
    );
...
export const POST = apiHandler(
  async (request: NextRequest, { params }: RouteParams) => {
    ...
    const [body, supabase] = await Promise.all([
      parseBody<AddTagsToItemInput>(request, addTagsToItemSchema),
      createClient(),
    ]);
    ...
    if (tagsError || !tags) {
      console.error(
        '[POST /api/library/[id]/tags] Error verifying tags:',
        tagsError,
      );
    ...
    if (tags.length !== body.tagIds.length) {
      return errors.badRequest(
        'One or more tags not found or do not belong to your organization',
      );
    }
    ...
    if (insertError) {
      console.error(
        '[POST /api/library/[id]/tags] Error adding tags:',
        insertError,
      );
    ...
    if (fetchError) {
      console.error(
        '[POST /api/library/[id]/tags] Error fetching updated tags:',
        fetchError,
      );
    }
    ...
    return successResponse({
      ...
      tags: (
        (updatedTags as unknown as ContentTagWithTagRow[] | null) ?? []
      ).flatMap((t) => (t.tags ? [t.tags] : [])),
    });
  },
);
...
export const PUT = apiHandler(
  async (request: NextRequest, { params }: RouteParams) => {
    ...
    const { id } = await params;
    const [body, supabase] = await Promise.all([
      parseBody<SetItemTagsInput>(request, setItemTagsSchema),
      createClient(),
    ]);
    ...
    if (body.tagIds.length > 0) {
      ...
      if (tagsError || !tags) {
        console.error(
          '[PUT /api/library/[id]/tags] Error verifying tags:',
          tagsError,
        );
        throw new Error('Failed to verify tags');
      }
    ...
    if (deleteError) {
      console.error(
        '[PUT /api/library/[id]/tags] Error removing existing tags:',
        deleteError,
      );
    ...
    if (body.tagIds.length > 0) {
      ...
      if (insertError) {
        console.error(
          '[PUT /api/library/[id]/tags] Error adding tags:',
          insertError,
        );
        throw new Error('Failed to update tags');
      }
    ...
    if (fetchError) {
      console.error(
        '[PUT /api/library/[id]/tags] Error fetching updated tags:',
        fetchError,
      );
    }
