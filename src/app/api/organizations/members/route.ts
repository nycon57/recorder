...
export const GET = rateLimit(
  ...
  apiHandler(async (request: NextRequest) => {
    ...
    if (error) {
      console.error(
        '[GET /api/organizations/members] Error fetching members:',
        error,
      );
...
export const POST = rateLimit(
  ...
  apiHandler(async (request: NextRequest) => {
    const [{ orgId, userId: inviterId }, bodyData] = await Promise.all([
      requireAdmin(),
      parseBody<z.infer<typeof inviteMemberSchema>>(
        request,
        inviteMemberSchema,
      ),
    ]);
    ...
    if (existingUserError) {
      console.error(
        '[POST /api/organizations/members] Error checking existing user:',
        existingUserError,
      );
      return errors.internalError();
    }
    ...
    if (invitationCheckError) {
      console.error(
        '[POST /api/organizations/members] Error checking existing invitation:',
    ...
    if (orgError) {
      console.error(
        '[POST /api/organizations/members] Error fetching organization:',
        orgError,
      );
      return errors.internalError();
    }
    ...
    if (countError) {
      console.error(
        '[POST /api/organizations/members] Error counting members:',
        countError,
      );
      return errors.internalError();
    }
    ...
    if (
      ...
      currentMemberCount &&
      currentMemberCount >= organization.max_users
    ) {
    ...
    if (createError || !invitation) {
      console.error(
        '[POST /api/organizations/members] Error creating invitation:',
        createError,
      );
      return errors.internalError();
    }
