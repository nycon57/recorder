...
export const GET = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    ...
    if (error || !member) {
      console.error(
        '[GET /api/organizations/members/[id]] Error fetching member:',
        error,
      );
...
export const PATCH = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    ...
    if (fetchError || !currentMember) {
      console.error(
        '[PATCH /api/organizations/members/[id]] Error fetching member:',
        fetchError,
      );
      return errors.notFound('Member');
    }

    const bodyData = await parseBody<z.infer<typeof updateMemberSchema>>(
      request,
      updateMemberSchema,
    );
    ...
    if (bodyData.role) {
      ...
      if (currentMember.role === 'owner' && requesterRole !== 'owner') {
        return errors.forbidden(
          'Only organization owners can modify owner roles',
        );
      }
      ...
      if (currentMember.role === 'owner' && bodyData.role !== 'owner') {
        ...
        if (countError) {
          console.error(
            '[PATCH /api/organizations/members/[id]] Error counting owners:',
            countError,
          );
        ...
        if (ownerCount && ownerCount <= 1) {
          return errors.badRequest(
            'Cannot change role: At least one owner must remain',
          );
        }
      }
    }
    ...
    if (
      bodyData.department_ids !== undefined &&
      bodyData.department_ids.length > 0
    ) {
    ...
    if (updateError || !updatedMember) {
      console.error(
        '[PATCH /api/organizations/members/[id]] Error updating member:',
        updateError,
      );
      return errors.internalError();
    }
...
export const DELETE = apiHandler(
  async (request: NextRequest, context: RouteContext) => {
    ...
    if (fetchError || !currentMember) {
      console.error(
        '[DELETE /api/organizations/members/[id]] Error fetching member:',
        fetchError,
      );
      return errors.notFound('Member');
    }
    ...
    if (currentMember.role === 'owner' && requesterRole !== 'owner') {
      return errors.forbidden(
        'Only organization owners can remove other owners',
      );
    }
    ...
    if (currentMember.role === 'owner') {
      ...
      if (countError) {
        console.error(
          '[DELETE /api/organizations/members/[id]] Error counting owners:',
          countError,
        );
        return errors.internalError();
      }
      ...
      if (ownerCount && ownerCount <= 1) {
        return errors.badRequest(
          'Cannot remove member: At least one owner must remain',
        );
      }
    }
    ...
    if (deleteError) {
      console.error(
        '[DELETE /api/organizations/members/[id]] Error deleting member:',
        deleteError,
      );
      return errors.internalError();
    }
