...
export const GET = apiHandler(
  async (
    ...
    if (error) {
      console.error(
        '[GET /api/organizations/departments/[id]/members] Error fetching members:',
        error,
      );
...
export const POST = apiHandler(
  async (
    ...
    const [{ orgId }, bodyData] = await Promise.all([
      requireAdmin(),
      parseBody<z.infer<typeof addUserToDepartmentSchema>>(
        request,
        addUserToDepartmentSchema,
      ),
    ]);
    ...
    if (insertError) {
      console.error(
        '[POST /api/organizations/departments/[id]/members] Error adding user:',
        insertError,
      );
...
export const DELETE = apiHandler(
  async (
    ...
    const [{ orgId }, bodyData] = await Promise.all([
      requireAdmin(),
      parseBody<z.infer<typeof removeUserFromDepartmentSchema>>(
        request,
        removeUserFromDepartmentSchema,
      ),
    ]);
    ...
    if (deleteError) {
      console.error(
        '[DELETE /api/organizations/departments/[id]/members] Error removing user:',
        deleteError,
      );
