...
export const GET = apiHandler(
  async (
    ...
    // Get member count
    const [{ count: memberCount }, { data: pathData }] = await Promise.all([
      supabase
        .from('user_departments')
        .select('*', { count: 'exact', head: true })
        .eq('department_id', id),
      supabase.rpc('get_department_path', {
        dept_id: id,
      }),
    ]);
...
export const PATCH = apiHandler(
  async (
    ...
    const [{ orgId }, bodyData] = await Promise.all([
      requireAdmin(),
      parseBody<z.infer<typeof updateDepartmentSchema>>(
        request,
        updateDepartmentSchema,
      ),
    ]);
    ...
    if (bodyData.parentId !== undefined) {
      ...
      if (bodyData.parentId) {
        ...
        if (isDescendant) {
          return errors.badRequest(
            'Cannot set parent to a descendant department (would create circular reference)',
          );
        }
      }
    }
    ...
    if (bodyData.parentId !== undefined)
      updateData.parent_id = bodyData.parentId;
    if (bodyData.defaultVisibility !== undefined)
      updateData.default_visibility = bodyData.defaultVisibility;
    ...
    if (updateError) {
      console.error(
        '[PATCH /api/organizations/departments/[id]] Error updating department:',
        updateError,
      );
...
export const DELETE = apiHandler(
  async (
    ...
    if (childError) {
      console.error(
        '[DELETE /api/organizations/departments/[id]] Error checking children:',
        childError,
      );
    ...
    if (reassignUsersTo) {
      ...
      if (targetError || !targetDept) {
        return errors.badRequest(
          'Target department for reassignment not found or not in your organization',
        );
      }
      ...
      if (reassignError) {
        console.error(
          '[DELETE /api/organizations/departments/[id]] Error reassigning users:',
          reassignError,
        );
      ...
      if (deleteUsersError) {
        console.error(
          '[DELETE /api/organizations/departments/[id]] Error removing users:',
          deleteUsersError,
        );
    ...
    if (deleteError) {
      console.error(
        '[DELETE /api/organizations/departments/[id]] Error deleting department:',
        deleteError,
      );
