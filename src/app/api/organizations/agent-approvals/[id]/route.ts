...
async function applyRoutingReviewDecision(input: {
  ...
  const previousState = parseRoutingReviewState(input.content.metadata);
  const approvedRoute =
  ...
  const nextMetadata = writeRoutingReviewState(input.content.metadata, routingState);
