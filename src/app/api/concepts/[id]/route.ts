...
import { createClient , supabaseAdmin } from '@/lib/supabase/admin';
...
export const PATCH = apiHandler(
  async (request: NextRequest, { params }: RouteParams) => {
    const [{ userId, orgId }, { id }, body] = await Promise.all([
      requireOrg(),
      params,
    ...
    const recordCorrection = (
      ...
      recordCorrectionFeedback(supabase, {
        orgId,
        userId,
        correctionValue,
        metadata,
    ...
    if (body.merge_into_id) {
      ...
      if (!mergeResult)
        return logAndFail('Merge concepts returned no data', null);
      ...
      await recordCorrection(
        `Merged "${result.source_name}" into "${result.target_name}"`,
        {
          action: 'merge',
    ...
    if (body.marked_incorrect) {
      const { data: deleteResult, error: deleteError } =
        await supabaseAdmin.rpc('delete_concept_with_mentions', {
          p_concept_id: id,
          p_org_id: orgId,
        } as Record<string, unknown>);
      ...
      if (!deleteResult)
        return logAndFail('Delete concept returned no data', null);
    ...
    if (body.name !== undefined && body.name !== concept.name) {
      ...
      updates.normalized_name = body.name
        .toLowerCase()
        .trim()
        .replace(/\s+/g, '_');
    ...
    if (
      body.concept_type !== undefined &&
      body.concept_type !== concept.concept_type
    ) {
      updates.concept_type = body.concept_type;
      changes.push(
        `Changed type from "${concept.concept_type}" to "${body.concept_type}"`,
      );
    }
