...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  const { targetUserId, userName, userRole } = body as {
    targetUserId?: string;
    userName?: string;
    userRole?: string;
  };
  ...
  // Verify targetUserId belongs to the same org
  ...
  if (userError) {
    console.error('[onboarding/plan] Error verifying target user:', userError);
    return errors.internalError();
  }
  if (!targetUser) {
