...
export const POST = apiHandler(
  async (request: NextRequest, { params }: RouteParams) => {
    ...
    const body = await parseBody<ApplyTagRequest>(
      request,
      applyTagRequestSchema,
    );
    ...
    if (recordingsError) {
      console.error(
        '[POST /api/tags/[id]/apply] Error fetching recordings:',
        recordingsError,
      );
    ...
    if (insertError) {
      console.error(
        '[POST /api/tags/[id]/apply] Error applying tags:',
        insertError,
      );
