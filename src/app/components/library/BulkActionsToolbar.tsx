...
export function BulkActionsToolbar(
  props: Parameters<typeof useBulkActionsToolbarImplementation>[0],
) {
  return useBulkActionsToolbarImplementation(props);
}

function useBulkActionsToolbarImplementation({
  selectedCount,
  onClearSelection,
  onDelete,
  onRestore,
  ...
  const handlePermanentDelete = async () => {
    ...
    console.log(
      '[BulkActionsToolbar] onPermanentDelete exists:',
      !!onPermanentDelete,
    );

    if (!onPermanentDelete) {
      console.log(
        '[BulkActionsToolbar] No onPermanentDelete callback provided',
      );
      return;
    }

    try {
      dispatch({ isPermanentlyDeleting: true });
  ...
  return (
    <>
          ...
          {mode === 'trash' ? (
            <>
              ...
              {onPermanentDelete && (
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => dispatch({ showPermanentDeleteDialog: true })}
            ...
            <>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => dispatch({ showDeleteDialog: true })}
                className="h-8 gap-1 sm:gap-2 text-red-500 hover:text-red-600 hover:bg-red-500/10"
      ...
      <Dialog
        ...
        onOpenChange={(open) => dispatch({ showDeleteDialog: open })}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => dispatch({ showDeleteDialog: false })}
              disabled={isDeleting}
            >
              Cancel
            </Button>
      ...
      <Dialog
        ...
        onOpenChange={(open) => dispatch({ showPermanentDeleteDialog: open })}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription className="text-red-500">
              ⚠️ This action cannot be undone. These items will be permanently
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => dispatch({ showPermanentDeleteDialog: false })}
