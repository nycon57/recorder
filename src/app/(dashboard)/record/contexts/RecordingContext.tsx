...
export function RecordingProvider(
  props: Parameters<typeof useRecordingProviderImplementation>[0],
) {
  return useRecordingProviderImplementation(props);
}

function useRecordingProviderImplementation({
  children,
}: {
  children: ReactNode;
}) {
  ...
  ] = useReducer(recordingReducer, initialRecordingState);

  const setCameraStream = (stream: MediaStream | null) =>
    updateRecordingState({ cameraStream: stream });
  const setMicrophoneStream = (stream: MediaStream | null) =>
    updateRecordingState({ microphoneStream: stream });
  const setScreenshareStream = (stream: MediaStream | null) =>
    updateRecordingState({ screenshareStream: stream });
  const setLayout = (nextLayout: RecordingLayout) =>
    updateRecordingState({ layout: nextLayout });
  const setCameraShape = (shape: CameraShape) =>
    updateRecordingState({ cameraShape: shape });
  const setCameraEnabled = (enabled: boolean) =>
    updateRecordingState({ cameraEnabled: enabled });
  const setMicrophoneEnabled = (enabled: boolean) =>
    updateRecordingState({ microphoneEnabled: enabled });
  const setIsRecording = (nextIsRecording: boolean) =>
    updateRecordingState({ isRecording: nextIsRecording });
  const setIsPaused = (nextIsPaused: boolean) =>
    updateRecordingState({ isPaused: nextIsPaused });
  const setRecordingBlob = (blob: Blob | null) =>
    updateRecordingState({ recordingBlob: blob });
  const setRecordingDuration = (duration: number) =>
    updateRecordingState({ recordingDuration: duration });
  const setPipWindow = (nextPipWindow: Window | null) =>
    updateRecordingState({ pipWindow: nextPipWindow });
  const setShowTeleprompter = (show: boolean) =>
    updateRecordingState({ showTeleprompter: show });
  const setCountdown = (count: number | null) =>
    updateRecordingState({ countdown: count });
  const setIsEntireScreenShared = (shared: boolean) =>
    updateRecordingState({ isEntireScreenShared: shared });
    ...
    const requestInitialMedia = async () => {
      ...
      if (microphoneEnabled && !microphoneStream) {
        ...
        try {
          ...
          console.log(
            '[RecordingContext] Initial microphone obtained:',
            micStream.id,
          );
          setMicrophoneStream(micStream);
        } catch (error) {
          console.error(
            '[RecordingContext] Failed to get initial microphone stream:',
            error,
          );
        }
      }
      ...
      if (layout !== 'screenOnly' && cameraEnabled && !cameraStream) {
        ...
        try {
          ...
          console.log(
            '[RecordingContext] Initial camera obtained:',
            camStream.id,
          );
          setCameraStream(camStream);
        } catch (error) {
          console.error(
            '[RecordingContext] Failed to get initial camera stream:',
            error,
          );
        }
      }
  ...
  useEffect(() => {
    ...
    console.log(
      '[RecordingContext] Layout changed:',
    ...
    const requestCameraForLayout = async () => {
      ...
      const needsCamera =
        layout !== 'screenOnly' && cameraEnabled && !cameraStream;
      ...
      if (needsCamera) {
        ...
        try {
          ...
          console.log(
            '[RecordingContext] Camera obtained after layout change:',
            camStream.id,
          );
          setCameraStream(camStream);
        } catch (error) {
          console.error(
            '[RecordingContext] Failed to get camera for layout:',
            error,
          );
        }
      }
  ...
  const requestMediaStreams = async (): Promise<boolean> => {
    try {
      ...
      if (layout !== 'cameraOnly') {
        ...
        console.log(
          '[RecordingContext] Screen share displaySurface:',
  ...
  const changeScreenshare = async (): Promise<boolean> => {
    try {
      ...
      console.log(
        '[RecordingContext] New screenshare obtained:',
        screenStream.id,
      );
      setScreenshareStream(screenStream);
      ...
      if (error instanceof Error && error.name === 'NotAllowedError') {
        console.log(
          '[RecordingContext] Screen sharing selection cancelled by user',
        );
        return false;
      }
  ...
  const startRecording = async () => {
    ...
    if (microphoneEnabled && !microphoneStreamRef.current) {
      console.log(
        '[RecordingContext] Requesting microphone before recording...',
      );
      try {
        const micStream = await navigator.mediaDevices.getUserMedia({
          audio: true,
  ...
  const beginRecording = () => {
    ...
    recordingTimerRef.current = setInterval(() => {
      ...
      // Warn at 25 minutes
      if (
        elapsed >= RECORDING_LIMITS.WARN_DURATION_MS &&
        !hasWarned25Min.current
      ) {
        hasWarned25Min.current = true;
        toast.warning(
          `Recording limit approaching: ${RECORDING_LIMITS_LABELS.WARN_DURATION} elapsed`,
          {
      ...
      if (elapsed >= RECORDING_LIMITS.MAX_DURATION_MS) {
        toast.error(
          `Recording automatically stopped: ${RECORDING_LIMITS_LABELS.MAX_DURATION} limit reached`,
        );
        stopRecording();
      }
    }, 1000);
    ...
    import('../services/composer').then(({ composeStreams }) => {
      ...
      if (videoTracks.length === 0) {
        console.error(
          '[RecordingContext] ❌ Composed stream has no video tracks!',
        );
      ...
      mediaRecorder.ondataavailable = (event) => {
        console.log(
          '[RecordingContext] ondataavailable fired, data size:',
          event.data.size,
        );
        if (event.data.size > 0) {
          chunks.push(event.data);
          console.log(
            '[RecordingContext] Chunk added, total chunks:',
            chunks.length,
            'total size:',
          ...
          console.warn(
            '[RecordingContext] ondataavailable fired but data is empty',
          );
        }
      };
      ...
      mediaRecorder.onstop = () => {
        console.log(
          '[RecordingContext] MediaRecorder onstop callback triggered',
        );
        ...
        try {
          composedStream.getTracks().forEach((track) => {
            if (track.readyState !== 'ended') {
              ...
              if (!isSourceTrack) {
                console.log(
                  '[RecordingContext] Stopping generated track:',
                  track.kind,
                  track.id,
                );
                track.stop();
                ...
                console.log(
                  '[RecordingContext] Preserving source track for reuse:',
                  track.kind,
                  track.id,
                );
              }
        ...
        console.log(
          '[RecordingContext] Recording blob created, size:',
          blob.size,
        );
      ...
      mediaRecorder.start();
      console.log(
        '[RecordingContext] MediaRecorder started, state:',
        mediaRecorder.state,
      );
  ...
  const stopRecording = () => {
    console.log(
      '[RecordingContext] stopRecording called, mediaRecorder exists:',
      !!mediaRecorderRef.current,
    );
    console.log(
      '[RecordingContext] mediaRecorder state:',
    ...
    if (isStoppingRef.current) {
      console.log(
        '[RecordingContext] Already stopping, ignoring duplicate call',
      );
      return;
    }
    ...
    if (
      ...
      try {
        ...
        console.error(
          '[RecordingContext] Error stopping mediaRecorder:',
          error,
        );
        isStoppingRef.current = false;
      }
      ...
      console.warn(
        '[RecordingContext] Cannot stop - mediaRecorder is null or already inactive',
      );
    }
  };
