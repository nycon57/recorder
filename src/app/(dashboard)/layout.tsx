    ...
    if (userData?.org_id) {
      const [planResult, settingsResult, pendingReviewCount] = await Promise.all([
        ...
        supabaseAdmin
          ...
          .eq('org_id', userData.org_id)
          .maybeSingle(),
        // TRIB-110: cached (60s) count of unified review-queue items for the
        // admin nav badge. Swallowed on failure — sidebar should never block
        // on this.
        getPendingReviewQueueCount(userData.org_id).catch(() => 0),
      ]);
