...
import '@/app/components/dashboard/tribora-dashboard.css';
...
export default async function DashboardLayout({
  ...
  try {
    ...
    if (userData?.org_id) {
      const [planResult, settingsResult, pendingReviewCount] =
        await Promise.all([
          userData.id
            ? supabaseAdmin
  ...
  return (
    <div className="tribora-dashboard">
      <SidebarProvider defaultOpen={true}>
        <AuroraSidebar
          role={userRole}
          isSystemAdmin={isSystemAdmin}
          hasOnboardingPlan={hasOnboardingPlan}
          hasDigestEnabled={hasDigestEnabled}
          wikiReviewCount={wikiReviewCount}
        />
        <SidebarInset>
          {/* Header with sidebar trigger and breadcrumbs */}
          <header className="trbd-shell-header sticky top-0 z-40 flex h-16 shrink-0 items-center gap-2 px-4">
            <SidebarTrigger className="-ml-1 text-muted-foreground hover:bg-muted/60 hover:text-foreground transition-colors" />
            <Separator orientation="vertical" className="mr-2 h-4 bg-border" />
            <Breadcrumbs />
          </header>

          {/* Main content area */}
          <main className="trbd-shell-main flex flex-1 flex-col gap-4">
            {children}
          </main>
        </SidebarInset>
      </SidebarProvider>
