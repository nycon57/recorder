...
function getVisibilityLabel(visibility: string): string {
  switch (visibility) {
    case 'private':
      return 'Private - Only specific members';
    case 'department':
      return 'Department - Department members only';
    case 'org':
      return 'Organization - All organization members';
    case 'public':
      ...
      <CardContent className="space-y-6 overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline" className="gap-1">
                ...
                <span>{getVisibilityLabel(department.default_visibility).split(' - ')[0]}</span>
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {getVisibilityLabel(department.default_visibility).split(' - ')[1]}
            </p>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Created {format(new Date(department.created_at), 'MMM d, yyyy')}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Calendar className="h-3 w-3" />
            <span>Updated {format(new Date(department.updated_at), 'MMM d, yyyy')}</span>
          </div>
        </div>
      </CardContent>
    </Card>
