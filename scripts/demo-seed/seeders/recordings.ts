...
import { loadRecordingFixtures, ZERO_VECTOR_1536 } from '../content-fixtures.js';
...
export async function seedRecordings(
  ...
  for (const rec of recordings) {
    ...
    if (opts.dryRun) {
      console.log(`[dry-run] Would upsert recording: ${rec.slug} (${rec.id})`);
      continue;
    }
    ...
    for (const tagName of rec.tagNames) {
