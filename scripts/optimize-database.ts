...
async function createOptimizationIndexes() {
  ...
  await Promise.all(
    Array.from(indexCreationQueries).map(async (index) => {
...
async function generateOptimizationReport() {
  ...
  const indexesByTable = indexes.reduce<Record<string, IndexInfo[]>>(
    (acc, idx) => {
  ...
  console.log(
    '   ✅ idx_users_clerk_id - For fast user authentication lookups',
  );
  console.log(
    '   ✅ idx_recordings_org_deleted_created - For recordings list queries',
  );
  console.log('   ✅ idx_jobs_status_created - For worker polling efficiency');
  console.log(
    '   ✅ idx_tags_org_deleted_name - For tag filtering and sorting',
  );
  console.log(
    '   ✅ idx_collections_org_deleted_name - For collection queries',
  );

  console.log('\n2. Query Optimization Tips:');
  console.log(
    '   • Always include org_id in WHERE clauses for multi-tenant isolation',
  );
  console.log(
    '   • Use partial indexes (WHERE deleted_at IS NULL) for soft delete patterns',
  );
  ...
  console.log(
    '   • Consider increasing lists parameter for larger datasets (>1M vectors)',
  );
  console.log(
    '   • Use probes parameter in queries to balance speed vs accuracy',
  );

  console.log('\n4. RLS Performance:');
  console.log('   • Ensure RLS policies use indexed columns (org_id, user_id)');
  console.log(
    '   • Consider using service role for background jobs to bypass RLS',
  );
  console.log(
    '   • Monitor pg_stat_user_tables for excessive sequential scans',
  );
  ...
  console.log(
    '   • Consider partitioning large tables (recordings, transcript_chunks)',
  );
